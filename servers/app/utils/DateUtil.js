...
//--------------------------------------
// 如何使用
// let DateUtil = require('../utils/DateUtil');    // src/dao文件夹
// let DateUtil = require('../src/utils/DateUtil');// routes文件夹
...
function leftDays(starttime) {
    ...
    let leftMillionSeconds = starttime.getTime() - curtime.getTime();
...
function between(curtime, starttime, endtime) {
    let start = diff2(curtime, starttime);
    let end = diff2(curtime, endtime);
...
function getLeftTimeToTheEndOfThisMonth() {
    let lastdate = getLastDateOfThisMonth();
    let timestamp = new Date(lastdate).getTime();
...
function isToday(timestamp) {
    ...
    let target = new Date(timestamp).getDate();
    if (today == target) {
        return true;
    }
    return false;
