...
class Pirate {
    constructor (record, sceneCfg) {
        this.reset();
        this._resetMission(record, sceneCfg);
    }
    ...
    _resetMission (record, sceneCfg) {
        if (!record || !Object.keys(record).length) {
            //随机产生海盗任务id
            let idx = Math.floor(Math.random() * sceneCfg.pirate_id.length);
            let id = sceneCfg.pirate_id[idx];
