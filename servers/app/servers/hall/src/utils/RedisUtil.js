...
function init(redis_client, pub_client, sub_client) {
    client = redis_client;
    pub = pub_client;
    sub = sub_client;
}

init(redisClient.cmd, redisClient.pubCmd,redisClient.subCmd);
...
function publish(channel, message, cb) {
    ...
    pub.publish(channel, message, function (err, res) {
        ...
        if (StringUtil.isString(message)) {
            logger.info(FUNC + "message length:", StringUtil.strLen(message));
        }
        else {
            logger.info(FUNC + "message:", message);
        }
