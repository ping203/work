const ErrCst = require('../buzz/cst/buzz_cst_error');
...
function showMeActivity(pool, data, cb) {
    ...
    DaoCommon.checkAccount(pool, token, async function (error, account) {
        ...
        await _initActive(account, cur_active_ids);
...
async function _initActive(account, active_id) {
    ...
    for (let idx in list) {
        ...
        type.push({repeat: repeat, condition: condition, value1: value1});
    }
...
function _isParamExist(param, err_info, cb) {
    if (param == null) {
        let extraErrInfo = {debug_info: "dao_activity._isParamExist()-" + err_info};
