...
function getRankgame(pool, data, cb) {
    ...
    let uid = data.token.split("_")[0];

    DaoCommon.checkAccount(pool, data.token, function (error, account) {
        if (error) {
            cb(error);
            return;
        }
    ...
    function doNextWithAccount(account) {
        logger.info(FUNC + 'account.match_unfinish:', account.match_unfinish);
...
function opReward(pool, data, account, cb) {
    ...
    if (idx == 3) {
        ...
        function rewardFirstBox() {
            ...
            let first_box_value = '{"stat":0,"timestamp":' + timestamp + ',"id":0}';
