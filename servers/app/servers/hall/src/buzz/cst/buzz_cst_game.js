...
// letiable
//==============================================================================
// let broadcast_server = { timestamp: _timestamp(), content: { txt: '好消息！《捕捞季》于2017年5月15开启正式测试，欢迎加入、参与、交流、反馈！', times: 3 } };
let broadcast_server = { timestamp: _timestamp(), content: { txt: '近期，有人以刷金币为诱饵索要QQ号和密码，请广大玩家勿要轻信，以免财物损失！', times: 1 } };
// android和ios分离
...
function _setBroadcastServer(new_broadcast) {
    ...
    RedisUtil.publish(redisKeys.CH.BROADCAST_SERVER, message);
}
...
function _setBroadcastGameEvent(new_broadcast) {
    ...
    RedisUtil.publish(redisKeys.CH.BROADCAST_GAME_EVENT, message);
...
function _setBroadcastFamousOnline(new_broadcast) {
    ...
    RedisUtil.publish(redisKeys.CH.BROADCAST_FAMOUS_ONLINE, message);
}
...
function _setBroadcastDraw(new_broadcast) {
    ...
    RedisUtil.publish(redisKeys.CH.BROADCAST_DRAW, message);
}
...
function _setBroadcastRewardPeople(new_broadcast) {
    ...
    RedisUtil.publish(redisKeys.CH.BROADCAST_REWARD_PEOPLE, message);
}
