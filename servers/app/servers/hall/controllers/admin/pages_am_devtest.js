...
var buzz_cst_gm_data = require('../../src/buzz/cst/buzz_cst_am_devtest');
...
function _makeVar() {
    ...
    data = _.extend(data, { title: 'Dev Test', expanded_id: 'menu_back_mgmt' });
    return data;
}

let exp = module.exports;
exp.get = async function (data) {
    return logicResponse.askEjs('admin/pages-am-devtest', _makeVar());
};
...
exp.post = async function (data) {
    return new Promise(function (resolve, reject) {
        buzz_admin_utils.checkTokenPost({
            ...
            if (err) {
                logger.error('pages-am-devtest err:', err);
                reject(err);
            }
            var params = _makeVar();
            params = _.extend(params, { user_auth: user_auth });
            resolve(logicResponse.askEjs("admin/pages-am-devtest", params));
        });
    });
};