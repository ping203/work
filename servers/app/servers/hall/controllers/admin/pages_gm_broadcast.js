...
var buzz_cst_gm_broadcast = require('../../src/buzz/cst/buzz_cst_gm_broadcast');
...
exp.get = async function (data) {
    return logicResponse.askEjs('admin/pages-gm-broadcast', _makeVar());
};
...
exp.post = async function (data) {
    return new Promise(function (resolve, reject) {
        buzz_admin_utils.checkTokenPost({
            ...
            if (err) {
                logger.error('pages-gm-broadcast err:', err);
                reject(err);
            }
            var params = _makeVar();
            params = _.extend(params, { user_auth: user_auth });
            resolve(logicResponse.askEjs("admin/pages-gm-broadcast", params));
        });
    });
};