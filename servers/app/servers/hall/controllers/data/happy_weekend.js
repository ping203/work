...
class HappyWeekend {
    ...
    getDataWithToken(dataObj, cb) {
        ...
        DaoCommon.checkAccount(global.mysqlPool, dataObj.token, function (error, account) {
            if (error) {
                cb(error);
                return;
            }
    ...
    //领奖
    check2GetReward(req, dataObj,cb) {
        const TAG = '领取周末狂欢奖励';
        DaoCommon.checkAccount(req.pool, dataObj.token, function (error, account) {
            if (error) {
                cb(error);
                return;
            }
    ...
    //更新捕鱼任意条进度
    saveFishingCount(req, dataObj, cb) {
...
async function _check2GetReward(data) {
    return new Promise(function(resolve, reject){
        let hw = new HappyWeekend();
        hw.check2GetReward({pool:global.mysqlPool}, data, function (err, result) {
            if (err) {
...
async function _saveFishingCount(data) {
    return new Promise(function(resolve, reject){
        let hw = new HappyWeekend();
        hw.saveFishingCount({pool:global.mysqlPool}, data, function (err, result) {
            if (err) {
