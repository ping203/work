...
function loadAll() {
    let sql = "SELECT id,uid,text,time,like_uids,like_count FROM `tbl_propose` ORDER BY like_count DESC limit 4";
    return new Promise(function (resole, reject) {
        mysqlConnector.query(sql, [], function (err, res) {
            if (err) {
                reject(err);
                return;
            }
...
function update(mid, uids, uid, like_count) {
    let like_uids = JSON.stringify([...uids, uid]);
