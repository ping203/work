...
const ERROR_OBJ = require('../buzz/cst/buzz_cst_error').ERROR_OBJ;
    ...
    const FUNC = TAG + "getCommonSql() --- ";
        ...
        if (ids.length > 0) {
            ...
            _addMail(pool, sql, sql_data, mail_id, function (err, results) {
                ...
                RedisUtil.publish(redisKey.CH.MAIL_RANK, message);
            });
        }
        else {
                ...
                if (mail_reward != null) {
                    ...
                    DaoReward.getReward(pool, account, mail_reward, function (err_get_reward, results_get_reward) {
                        ...
                        _updateDb4AccountMailBox(pool, account.id, function (err, result) {
                            ...
                            if (goldGain > 0) {
                                // yDONE: 金币记录日志
                                logger.error(FUNC + uid + "领取邮件发放的金币");
                                logGold.push({
                                    account_id: uid,
                                    log_at: new Date(),
                                    gain: goldGain,
...
function _didMailList(pool, data, cb) {
    ...
    DaoCommon.checkAccount(pool, token, function (err, account) {
        ...
        //logger.info("_didMailList()--------------mail_box:", mail_box);
        let mail_detail = [];
        // 需要查找到所有的邮件内容做成结构化数据块返回
        if (mail_box != null) {
            logger.info("------------------mail_box.length:", mail_box.length);
        }
        if (mail_box != null && mail_box.length > 0) {
            ...
            _getMailsDetail(mail_box, function (err, mails) {
                ...
                for (let i in mail_box) {
                    ...
                    logger.info("cache:", CacheMail.cache());
                ...
                logger.info("_didMailList()--------------mail_detail:", mail_detail);
                cb(null, mail_detail);
            });
        }
        else {
