...
function _getBankruptcyCompensation(pool, data, cb) {
    ...
    function doNextWithAccount(account) {
        ...
        if (vipInfo) {
            logger.info(FUNC + `VIP信息:`, vipInfo);
            let vip_alms_times = vipInfo.vip_alms_times;
            vip_alms_value = vipInfo.vip_alms_value;
            ...
            logger.info(FUNC + `没有找到VIP信息`);
        }
        
        // 玩家领取救济金数量获取
        let gold = account.gold;
        if (gold > 0) {
            logger.info(FUNC + `玩家${uid}的金币不为0(实际拥有${gold})`);
            cb(new Error('不满足领取救济金条件'));
            return;
        }
