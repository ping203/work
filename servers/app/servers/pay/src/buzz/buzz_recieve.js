...
function _didChangeInKind(req, dataObj, cb) {
    ...
    function doNextWithAccount(account) {
        ...
        function afterCheck() {
            ...
            //     });
            // });
            buzz_cik.cikReduce(req, data, function (err, values) {
                prepareReturn();
            });
            ...
            function prepareReturn() {
                ...
                buzz_cik.addCikOrder(req, data, function (err, result) {
                    myDao.addChangeLog(result, function () {
 ...
 * @param {*} cb 
 */
function _buyVipGift(req, dataObj, cb) {
    ...
    function doNextWithAccount(account) {
        ...
        if (buyLevel > account.vip) {
            var extraErrInfo = { debug_info: 'Recharge and level up your VIP!' };
            cb(ObjUtil.merge(extraErrInfo, ERROR_OBJ.VIP_NOT_ENOUFH));
            return;
        }

        var accountCoin = account[coinType];
        if (giftPrice > accountCoin) {
            var extraErrInfo = { debug_info: 'Coin not enough, recharge first!' };
        ...
        if (ArrayUtil.contain(account.vip_gift, buyLevel)) {
            var extraErrInfo = {debug_info: 'Already buy it, do not try to buy it again!'};
        ...
        BuzzUtil.useCoin(account, coinId, giftPrice, function(err, res) {
            BuzzUtil.putIntoPack(req, account, item_list, function(rewardinfo) {
                ...
                GameLog.addGameLog(item_list, account, SCENE.VIPGIFT_BUY, "购买VIP礼包获取");
            });
        });
    }
}
