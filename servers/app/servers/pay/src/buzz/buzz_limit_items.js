...
const item_item_cfg = require('../../../../utils/imports').GAME_CFGS.item_item_cfg;
            ...
            for (let i = 0; i < reward.length; i++) {
                ...
                if (IT_CFG) {
                    ...
                    if (ltype === 1 || ltype === 2) {
                        ...
                        logger.info('-itemId - itemId = ', itemId);
 ...
 * 剔除限时道具
 */
function clearLimitItem (uid, itemId, gotAt, clearCount, doneFunc) {
    let pk = LIMIT_ITEM_HK + itemId;
    RedisUtil.hget(pk, uid, function (err, ret) {
        if (ret) {
            ret = JSON.parse(ret);
